...
function Spectcl(options){
    ...
    /**
     * Child object. Mutually exclusive with _stream.
     */
    this.child = null

    /**
     * Duplex stream object. Mutually exclusive with child.
     */
    this._stream = null
...
Spectcl.prototype.spawn = function (command, cmdParams, cmdOptions, spawnOptions){
    ...
    if(this._stream){
        throw new Error('Spawning child not supported while operating on duplex stream')
    }
    var self = this
...
/**
 * Creates the Expect session on a duplex stream
 * @param {Object} stream - Duplex stream to operate on.
 * @returns {undefined}
 */
Spectcl.prototype.stream = function (stream){
    if(this.child){
