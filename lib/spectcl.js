...
function Spectcl(options){
     ...
     * @property {number} matchMax - Max length of cache. Defaults to 2000
     */
    ...
    /**
     * Child object. Mutually exclusive with stream behavior.
     */
    this.child = null

    /**
     * Duplex stream object. Mutually exclusive with child behavior.
     */
    this._stream = null

    /**
     * Readable stream object. Mutually exclusive with child behavior.
     */
    this._readable = null

    /**
     * Writable stream object. Mutually exclusive with child behavior.
     */
    this._writable = null
...
Spectcl.prototype.spawn = function (command, cmdParams, cmdOptions, spawnOptions){
    ...
    if(this._stream || this._readable || this._writable){
        throw new Error('Spawning child not supported while operating on streams')
    }
    var self = this
...
/**
 * Creates the Expect session on a duplex stream or pair of readable and writable
 * @param {Object} stream - Duplex or readable stream to operate on.
 * @param {Object} writable - Writable stream to operate on, when passed with a readable-only stream.
 * @returns {undefined}
 */
Spectcl.prototype.stream = function (stream, writable){
    if(this.child){
        throw new Error('Spawning child not supported when operating on duplex stream!')
    }
    if(this._stream || this._readable || this._writable){
    ...
    this.cacheStream = through(onData, function(){
        this.queue(null)
    })

    this.cacheStream.pause()
    debug('[stream] created cacheStream')

    if(this._stream){
        this._stream.pipe(this.cacheStream)
        /**
        * `child-pty` stdout/in streams do not emit `end` or `close`.
        * Watch for child to `close` (stdio streams terminated), and
        * queue an end event onto the cache stream.
        */
        ...
        this._readable.pipe(this.cacheStream)
        /**
        * `child-pty` stdout/in streams do not emit `end` or `close`.
        * Watch for child to `close` (stdio streams terminated), and
        * queue an end event onto the cache stream.
...
Spectcl.prototype.send = function(data, cb){
    var self = this
    debug('[send] writing %s to session',util.inspect(data))

    var writeErrListener = function(err){
        debug('[send] err %s',err)
    }
    if(self._stream){
        self._stream.on('error', writeErrListener)
        self._stream.write(data, function(err){
            if(err){
                self._stream.removeListener('error',writeErrListener)
            ...
            self._stream.removeListener('error',writeErrListener)
            debug('[send] wrote to stream')
            if(cb){
                return cb()
            }
        })
    } else if(self._readable) {
        self._writable.on('error', writeErrListener)
        self._writable.write(data, function(err){
            if(err){
                self._writable.removeListener('error',writeErrListener)
                debug('[send] error: %s',err)
                if(cb){
                    return cb(err)
                }
...
Spectcl.prototype.sendEof = function(cb){
    ...
    if(self.child.stdout.ttyname){
        ...
        debug('[sendEof] end stream')
        self._stream.end()
    } else if(self._writable){
        debug('[sendEof] end stream')
        self._writable.end()
    } else {
